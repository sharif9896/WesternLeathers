...
const Product = () => {
  ...
  const addtocart = async (id, size) => {
    ...
    if (size.length === 0 && id === productdata._id) {
      if (cartdata[id]) {
        if (cartdata[id][size]) {
          cartdata[id][size] += 1;
        } else {
    ...
    if (!size) {
      toast.warning("If You have Size in your product please Select the Size, If You have not Size in your product no matter your product is added to cart.");
      return;
    }
    else {
